...
class _GIAtspiRuntime:
    ...
    def __init__(self) -> None:
        ...
        try:
            ...
            raise RuntimeError(
                "could not initialize the AT-SPI accessibility registry"
            ) from exc
        if status != 0:
            raise RuntimeError(
    ...
    def children(self, element: Any) -> list[Any]:
        count = _integer(
            _call(element, "get_child_count", "getChildCount")
        )
        if count is None:
            return []
        children: list[Any] = []
        for index in range(min(count, self._MAX_NODES)):
            try:
    ...
    def element_at_point(self, x: float, y: float) -> Any:
        ...
        matches: list[Any] = []
        for application in self.children(self.desktop):
            for surface in self.children(application) or [application]:
                bounds = self.bounds(surface)
                if bounds is None:
                    continue
                if not (
                    bounds.left <= point_x < bounds.right
                    and bounds.top <= point_y < bounds.bottom
                ):
                    continue
                if not self._state_contains(surface, "ACTIVE"):
                    continue
    ...
    def focused_element(self) -> Any:
        ...
        while stack and visited < self._MAX_NODES:
            ...
            stack.extend(reversed(self.children(element)))
        if stack:
            return None
    ...
    def process_id(self, element: Any) -> int | None:
        ...
        for _ in range(MAX_STRUCTURAL_ANCESTRY_DEPTH):
            process_id = _integer(
                _call(current, "get_process_id", "getProcessId")
            )
    ...
    def action_names(self, element: Any) -> list[str] | None:
        ...
        if action is None:
            return None
        count = _integer(
            _call(action, "get_n_actions", "getNActions")
        )
        if count is None:
            return None
        names = []
...
def _fields(runtime: Any, element: Any) -> dict[str, Any]:
    ...
    return {
        "automation_id": _text(
            attributes.get("accessible-id") or attributes.get("id")
        ),
        "role": role,
        ...
        "name": name,
        "class_name": _text(
            attributes.get("class") or attributes.get("toolkit")
        ),
...
class LinuxATSpiStructuralObserver:
    ...
    def observe(
        self, request: StructuralObservationRequest
