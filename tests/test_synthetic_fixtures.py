...
def _is_the_reference_builder() -> bool:
    ...
    The determinants are the ones the provenance `builder` block names: this
    repository's writer code plus the required SQLite and SQLAlchemy versions.
    The Python version is not among them. CPython 3.10.20, 3.11.15 and 3.12.13,
    each carrying SQLite 3.53.1 and SQLAlchemy 2.0.52, regenerate all ten
    committed artifacts identically, so every lane whose libraries match runs
    the byte check and the tamper tests rather than skipping them.

    A tamper test is meaningful only on a matching builder: elsewhere a raised
    `generated bytes are stale` could report the builder rather than the tamper.
    """
    import sqlalchemy

    return (sqlite3.sqlite_version, sqlalchemy.__version__) == (
        REQUIRED_SQLITE_VERSION,
        REQUIRED_SQLALCHEMY_VERSION,
    )
...
def test_public_fixtures_match_the_source_generator_byte_for_byte() -> None:
    """Every lane asserts one of the two outcomes, and none asserts nothing.

    A matching builder regenerates the committed bytes and compares them. A
    non-matching builder cannot reproduce them, so it asserts that the
    generator refuses to write instead of emitting bytes it cannot certify.
    """
    if _is_the_reference_builder():
        check_generated(
